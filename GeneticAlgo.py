...
class GeneticAlgorithm(object):
    ...
    def calculate_population_fitness(self,trainInput,trainOutput,net):
        """Calculate the fitness of every member of the given population using
        the supplied fitness_function.
        """
        for individual in self.current_generation:
            individual.fitness = self.fitness_function(
                individual.genes, trainInput,trainOutput,net)
    ...
    def create_first_generation(self, trainInput,trainOutput,net):
        """Create the first population, calculate the population's fitness and
        rank the population by fitness according to the order specified.
        """
        self.create_initial_population()
        self.calculate_population_fitness(trainInput,trainOutput,net)
        #print "fitness of first gen computed"
        self.rank_population()

    def create_next_generation(self, trainInput,trainOutput,net):
    ...
    def run(self, popSize,Iter,trainInput,trainOutput,net):
    	convergence=[]
    	s=solution()
    	s.optimizer="GA"
        ...
        self.create_first_generation(trainInput,trainOutput,net)
        for _ in range(1, self.generations+1):
            self.create_next_generation(trainInput,trainOutput,net)
            best_fit, bestIndividual = self.best_individual()
            convergence.append(best_fit)
            print(s.optimizer, '  ',  _ ,' > best:',best_fit)
