...
def gso(func_obj, lb, ub, agents_number, epochs, trainInput,trainOutput, net, dim):
#def gso(agents_number, dim, func_obj, epochs, step_size, r0, rs, b, k_neigh, dims_lim = [-4,4], random_step=False, virtual_individual = False, individual_updated=None, program_ends = None):
	...
	for epoch in range(epochs):
		...
		for i in range(agents_number):
			li = luciferins[i]
			if epoch == 0:
				fitness[i]=1-func_obj(glowworms[i],trainInput,trainOutput,net)
		...
		print(s.optimizer, ' at iteration ', epoch,'> best fitness:',1-best_fitness_history[-1])
